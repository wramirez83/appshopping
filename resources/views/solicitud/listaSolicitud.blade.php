...
<div class="col-lg-12">
              ...
              <tbody>
                ...
                <form class="" action="index.html" method="post">
                  ...
                  <tr class="typo-articles">
                    ...
                    <td>{{ $llaveProductos->precio }}</td>
                    <td>{{ $llaveProductos->cantidad }}</td>
                    <td>{{ $t = $t + ($llaveProductos->cantidad * $llaveProductos->precio) }}</td>
                    <td><i class="fa fa-eraser" onclick="eliminar({{ $llaveProductos->id_productos_solicitudes }})" name="{{ $llaveProductos->id_productos_solicitudes }}"></i></td>
                  </tr>
                  @endforeach
                </form>
              </tbody>
    ...
    $("#botonBuscarProducto").click(function(){
      ...
      unidad_medida: unidad_medidaB},function( datosI )
      {
        ...
        for(i2 = 0; i2 < data.length; i2++)
        {
          ...
          cell7.innerHTML = '<button onclick="asignarProducto(this)" name="' + i2 + '"><i class="fas fa-bolt" type="button"></i></button'
  ...
  {
    if(parseFloat(document.getElementById('totalPedido').innerHTML) > 0)
    {
      var tablaP = document.getElementById("productosSel");
      var fila = tablaP.rows.length;
  ...
  function eliminar(item)
  {
    ...
    if(confirm('Desea Quitar Elemento de la Solicitud'))
    {

      window.location.href = "/eliminarProductoSolicitud/" + id + "/" + '{{ $solicitud->id_solicitud }}';
    }
  }
  </script>
  ...
  <form class="" action="{{ Route('actualizarSolicitud') }}" method="post" name="formularioJS">
    {{ CSRF_FIELD() }}
